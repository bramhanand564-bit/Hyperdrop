...
import { AIConnectionRegistry, AI_CONNECTION_TYPES } from './AIProviderRegistry';

const SETTINGS_KEY = 'nax.ai.settings.v1';
const SECRET_PREFIX = 'nax.ai.secret.';
const EMPTY = { activeConnectionId: null, connections: [] };
function makeId() { return `ai_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`; }
...
async function readSettings() {
  try {
    ...
    return { activeConnectionId: parsed?.activeConnectionId || null, connections: Array.isArray(parsed?.connections) ? parsed.connections : [] };
  } catch (error) { console.error('AISettingsService.readSettings:', error); return EMPTY; }
}
async function writeSettings(settings) { await AsyncStorage.setItem(SETTINGS_KEY, JSON.stringify(settings)); }
async function setSecret(id, apiKey) {
  const key = `${SECRET_PREFIX}${id}`;
  if (apiKey) await SecureStore.setItemAsync(key, apiKey); else await SecureStore.deleteItemAsync(key).catch(() => {});
}
async function getSecret(id) { if (!id) return ''; try { return await SecureStore.getItemAsync(`${SECRET_PREFIX}${id}`) || ''; } catch (_) { return ''; } }

const AISettingsService = {
  async listConnections({ includeSecrets = false } = {}) {
    const settings = await readSettings();
    if (!includeSecrets) return settings;
    return { ...settings, connections: await Promise.all(settings.connections.map(async connection => ({ ...connection, apiKey: await getSecret(connection.id) }))) };
  },
  async getConnection(id, { includeSecret = true } = {}) {
    ...
    return includeSecret ? { ...connection, apiKey: await getSecret(id) } : connection;
  },
  ...
  async saveConnection(input = {}) {
    ...
    const type = input.type || AI_CONNECTION_TYPES.OPENAI_COMPATIBLE;
    const baseUrl = String(input.baseUrl || '').trim().replace(/\/$/, '');
    const model = String(input.model || '').trim();
    const models = AIConnectionRegistry.normalizeModels(input.models, model);
    if (!name) throw new Error('Connection name is required.');
    if (type !== AI_CONNECTION_TYPES.ON_DEVICE && !baseUrl) throw new Error('API base URL is required.');
    ...
    const connection = {
      id, name, type, baseUrl: baseUrl || 'on-device://',
      model: model || models[0], models,
      createdAt: existingIndex >= 0 ? settings.connections[existingIndex].createdAt : Date.now(),
      updatedAt: Date.now(),
    };
    if (existingIndex >= 0) settings.connections[existingIndex] = connection; else settings.connections.push(connection);
  ...
  async setActiveConnection(id) {
    ...
    settings.activeConnectionId = id; await writeSettings(settings);
  ...
  async deleteConnection(id) {
    ...
    await writeSettings(settings); await setSecret(id, ''); return settings;
  },
