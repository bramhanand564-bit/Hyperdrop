...
class CognitiveTrajectory:
    ...
    def is_useful(self)->bool:return all(bool(str(x).strip()) for x in (self.question,self.intent,self.next_action,self.response_strategy))
def _clean(value:Any,limit:int=1200)->str:return re.sub(r"\s+"," ",str(value or "")).strip()[:limit]
def _extract_json(text:str)->dict[str,Any]:
    try:
        v=json.loads(text.strip()); return v if isinstance(v,dict) else {}
...
def quality_score(item:CognitiveTrajectory)->float:
    ...
    return round(min(1.0,.75*filled+.25*diversity),3)
def extract_record(record:dict[str,Any],min_score:float=.65)->CognitiveTrajectory|None:
    q=record.get("question") or record.get("prompt") or record.get("input"); payload=record.get("cognition") or record.get("trajectory") or record
    if not q or not isinstance(payload,dict):return None
    item=normalize_teacher_output(q,payload); return item if quality_score(item)>=min_score else None
def build_teacher_prompt(question:str)->str:
    return ("Analyze the user's question as a cognitive teacher. Do not solve it with a long factual answer. Return ONLY valid JSON with these fields: "+", ".join(COGNITIVE_FIELDS)+". Describe reusable thinking, not a fact dump or code solution. Keep each field concise. question="+question)
...
def extract_jsonl(input_path:str,output_path:str,endpoint:str|None=None,model:str|None=None,min_score:float=.65)->int:
    ...
    with Path(input_path).open(encoding="utf-8") as source,out.open("w",encoding="utf-8") as sink:
        for line in source:
            ...
            record=json.loads(line); q=record.get("question") or record.get("prompt") or record.get("input")
            if not q:continue
            item=normalize_teacher_output(q,call_teacher(endpoint,model,q)) if endpoint and model else extract_record(record,min_score)
            if item is None or quality_score(item)<min_score:continue
