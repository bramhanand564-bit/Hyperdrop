...
class JobWorker:
    ...
    def run_once(self, handler) -> WorkerResult | None:
        ...
        self.checkpoints.save(Checkpoint(job.id,"running","running",job.progress,{"attempts":job.attempts}))
        try:
            progress=handler(job)
            job.progress=max(0.0,min(1.0,float(progress)))
            job.state=JobState.COMPLETED if job.progress>=1.0 else JobState.PAUSED
            self.checkpoints.save(Checkpoint(job.id,"worker",job.state.value,job.progress))
        except Exception as exc:
            job.state=JobState.FAILED
            job.error=str(exc)
            self.checkpoints.save(Checkpoint(job.id,"worker","failed",job.progress,{"error":job.error}))
