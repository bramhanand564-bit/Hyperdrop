...
export class TriggerManager {
  ...
  register(workflow){
    ...
    const unsubscribe=EventBus.on(eventType, async payload=>{
      ...
      try { const result=await WorkflowRunner.run(workflow,payload); await ExecutionLogger.log({workflowId:workflow.id||null,eventType,result,durationMs:Date.now()-started}); }
