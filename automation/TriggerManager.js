...
export class TriggerManager {
  ...
  register(workflow){
    ...
    const unsubscribe=EventBus.on(eventType, async payload=>{
      if (payload?.workflowId && payload.workflowId !== workflow.id) return;
      ...
      catch(error){ await ExecutionLogger.log({workflowId:workflow.id||null,ownerId:workflow.ownerId||null,eventType,status:'failed',error:error?.message,durationMs:Date.now()-started}); }
    });
